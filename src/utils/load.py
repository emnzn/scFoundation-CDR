...
from .pretrainmodels.select_model import select_model
...
def load_model(best_ckpt_path, device):
    model_data = torch.load(best_ckpt_path,map_location=device,weights_only=True)
...
def load_model_frommmf(best_ckpt_path, key='gene'):
    model_data = torch.load(best_ckpt_path,map_location='cpu',weights_only=True)
